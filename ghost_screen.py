...
class iOSBackend(GhostScreen):
    ...
    def _render_frame(self, w, h):
        from PIL import Image, ImageDraw
        bg = tuple(int(self.cfg["colors"]["bg"].lstrip("#")[i:i+2], 16)
                   for i in (0, 2, 4)) + (255,)
        ...
        c = self.cfg["colors"]
        cx, cy = w // 2, h // 2
        fy = math.sin(t * 2) * self.cfg["float_amplitude"]
        gy = cy + fy
        scale = min(w, h) * self.cfg["ghost_scale"]
        self._draw_vignette(draw, t, c)
        self._draw_grid(draw, t, cx, cy, c["grid"])
        self._draw_rings(draw, t, cx, gy, scale, c)
        self._draw_ghost(draw, t, cx, gy, scale, c)
    ...
    def _draw_vignette(self, draw, t, c):
        ...
        for i in range(5):
            r = 1 - i * 0.15
            a = int(i * 0.03 * self.cfg["opacity"] * 255)
            x0 = int(w * (1 - r) / 2)
            y0 = int(h * (1 - r) / 2)
            x1 = int(w * (1 + r) / 2)
            y1 = int(h * (1 + r) / 2)
    ...
    def _draw_hud(self, draw, t, cx, cy, scale, color):
        ...
        for bx, by, dx, dy in corners:
            ...
            scan_col = _hex_to_rgba_pil(color, 48)
            sx = cx - spread
            for i in range(int(spread / 6)):
                sy = cy - spread + (t * 60 + i * 12) % (spread * 2)
                draw.line([(sx, sy), (sx + 3, sy + 3)], fill=scan_col, width=1)

    def run(self):
        import ui
        sw, sh = ui.get_screen_size()
        self.sw, self.sh = sw, sh
        iv = ui.ImageView(frame=(0, 0, sw, sh))
        v = ui.View(frame=(0, 0, sw, sh))
        ...
        def anim():
            if self._quit:
                ...
                return
            if self._visible:
                img = self._render_frame(sw, sh)
                buf = io.BytesIO()
                img.save(buf, 'PNG')
                iv.image = ui.Image.from_data(buf.getvalue())
            ui.delay(self.cfg["frame_delay"] / 1000.0, anim)
        anim()
...
def create_ghost_screen(cfg=None):
    try:
        import ui as _ui
        print("  Backend: iOS (ui + PIL)")
        return iOSBackend(cfg)
    except ImportError:
        pass
