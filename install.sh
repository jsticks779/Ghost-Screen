...
# ── Shortcut setup via ghost-screen itself ──────────────────────────────

SHORTCUT_OK=""
SHORTCUT_MSG=""

if command -v "$CMD" &>/dev/null; then
    echo "    Setting Ctrl+3 shortcut..."
    SHORTCUT_OUTPUT=$("$CMD" --shortcut Ctrl+3 2>&1) && {
        SHORTCUT_OK=1
        echo "    $SHORTCUT_OUTPUT"
        SHORTCUT_MSG=$(echo "$SHORTCUT_OUTPUT" | grep -oP '(?<=  ).*' | tail -1)
    } || {
        echo "    $SHORTCUT_OUTPUT"
